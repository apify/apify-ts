...
[Crawlee CLI](https://www.npmjs.com/package/@crawlee/cli). This command line tool allows you to create, run and manage Crawlee
...
TODO describe purging by default and how to disable it
...
This configuration option is available in all crawler classes and you can use it to limit the number of `Request`s the crawler should process. It's
...
If you were paying attention carefully in the previous chapter, we said that we would show you a way to enqueue new `Request`s with a single function
...
A pseudo-URL may include any number of bracketed regular expressions, so you can compose much more complex matching logic. The following Pseudo URL will match the items in the store even if the links use the non-secure `http` protocol, omit the `www` from the hostname or use a different TLD.
...
The answer to the first one is a definitive no. `RequestList` is immutable and once you create it, you cannot add or remove `Request`s from it. The
...
> processed by the `RequestQueue` afterwards. You can, of course, enqueue the `Request`s to the queue manually, but that would take some boilerplate
...
The code should look pretty familiar to you. It's a very simple `requestHandler` where we log the currently processed URL to the console and
...
and by calling <ApiLink to="core/class/KeyValueStore#getInput">`KeyValueStore.getInput()`</ApiLink> you retrieve the value from the `KeyValueStore`.
...
We won't go to great lengths here to talk about `utils.log`, because you can read <ApiLink to="core/class/Log">it all in the documentation</ApiLink>, but there's just

...
A prompt will be shown, asking us to choose a template. Let's choose the first one called `Crawlee playwright template [TypeScript]`. The command will now create a new directory in our current working directory, called `my-new-project`, create a `package.json` in this folder and install all the necessary dependencies. It will also add example source code that we can immediately run.

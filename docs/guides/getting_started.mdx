...
```ts
import { Actor } from 'apify';
import { RequestQueue } from '@crawlee/cheerio';
...
make your life easier. One of the available functions is `enqueueLinks()` which encapsulates the whole enqueueing process and even adds
...
```ts
import { Actor } from 'apify';
import { CheerioCrawler, CheerioCrawlingContext } from '@crawlee/cheerio';

await Actor.main(async () => {
    const requestHandler: CheerioCrawlingContext = async ({ request, $, enqueueLinks }) => {
        const title = $('title').text();
        console.log(`The title of "${request.url}" is: ${title}.`);

...
Crawlee storage is managed by [@crawlee/memory-storage](https://github.com/apify/apify-ts/tree/master/packages/memory-storage) NPM package. During the crawler run all information is stored in memory, while also being off-loaded to the local files in respective storage type folders.
...
Default storages are not purged before the crawler starts if not specified otherwise. If you want to start the run with the empty default storages, you should use the `purge()` function before running the crawler as follows:
...
Calling this function will clean up the following directories:

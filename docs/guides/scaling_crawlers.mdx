...
These control how many parallel requests can be run at any time. By default, crawlers will start with one parallel request at a time and scale up over time to a maximum of 1000 requests at a time.

:::caution Don't set `minConcurrency` too high!

Setting this option too high compared to the available system resources will make our crawler run extremely slow or might even crash.
...
This section is super advanced and, unless we test the changes extensively and know what we're doing, it's better to leave these options to the default configurations provided, as they are most likely going to work for us without much fuss.
...
If we're planning on modifying these, make sure the value we set is not `0`, and that it is positive!
...
It's recommended we keep this value between `5` and `20` seconds.

:::

:::caution Be careful with how low, or high, we set this option
...
With that said, if we configure this alongside [`scaleUpStepRatio` and `scaleDownStepRatio`](#scaleupstepratio-and-scaledownstepratio), we could make our crawler scale up at a slower interval, but with more requests at a time when it does.
...
This option can be set by specifying [`maxRequestsPerMinute`](#maxrequestsperminute) in our crawler options too, as it is a shortcut for visibility and ease of access.

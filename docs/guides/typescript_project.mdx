...
// getInput returns Promise<Dictionary | string | Buffer | null> by default,
// we can narrow that via its generic type argument
const input = await KeyValueStore.getInput<MySchema>();

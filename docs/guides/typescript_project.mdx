...
const input: MySchema = (await KeyValueStore.getInput()) as any; // getInput returns Promise<Object<string, *>|string|Buffer|null> here

if (!input?.expectedParam1) {
    // input is MySchema now and we can check in a type-safe way
    throw new Error('Missing expectedParam1');
}
```

There are also other places where we need to explicitly provide our interface/type, like in Dataset iterators (`map`, `reduce`, `forEach`):

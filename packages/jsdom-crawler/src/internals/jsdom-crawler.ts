...
import { Awaitable, Dictionary, entries, parseContentTypeFromResponse } from '@crawlee/utils';
 ...
 * @category Crawlers
 */
export class JSDOMCrawler<JSONData = Dictionary> extends BasicCrawler<
...
> {
    ...
    constructor(options: CheerioCrawlerOptions<JSONData>) {
        ow(options, 'CheerioCrawlerOptions', ow.object.exactShape(JSDOMCrawler.optionsShape));
    ...
    protected override async _runRequestHandler(crawlingContext: CheerioCrawlingContext<JSONData>) {
        ...
        if (!request.skipNavigation) {
            ...
            const { window, body, contentType, response } = await this._parseResponse(request, crawlingContext.response!);
            tryCancel();
    ...
    protected async _parseResponse(request: Request, responseStream: IncomingMessage) {
        ...
        if (statusCode! >= 500) {
            ...
            return ({ window: jsdom.window, response, contentType });
        } else {
...
/** @internal */
export async function cheerioCrawlerEnqueueLinks({ options, window, requestQueue, originalRequestUrl, finalRequestUrl }: EnqueueLinksInternalOptions) {
    if (!window) {
    ...
    const urls = extractUrlsFromCheerio(window, options?.selector ?? 'a', baseUrl);
 ...
 * @ignore
 */
function extractUrlsFromCheerio(window: DOMWindow, selector: string, baseUrl?: string): string[] {
    return [
        // @ts-expect-error Incorrect @types/jsdom
